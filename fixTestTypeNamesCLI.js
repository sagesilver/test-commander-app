// Node.js script to fix test type names using Firebase CLI authentication
// Run this locally with: node fixTestTypeNamesCLI.js
...
// Initialize Firebase Admin SDK using CLI authentication
try {
  // Get the access token from Firebase CLI
  const accessToken = execSync('firebase login:ci --no-localhost', { encoding: 'utf8' }).trim();
  ...
  console.log('✅ Firebase Admin SDK initialized successfully using CLI authentication');
} catch (error) {
  console.error('❌ Failed to initialize Firebase Admin SDK:', error.message);
  console.log('\n📋 Alternative Setup:');
  console.log('1. Make sure you are logged into Firebase CLI: firebase login');
  console.log('2. Or use the original script with service account key');
  process.exit(1);
}
